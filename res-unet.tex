...
\node[canvas is zy plane at x=0] (input) at (-3,0,0) {\includegraphics[width=8cm,height=8cm]{figs/inputs.png}};
...
\pic[shift={(2.1,0,0)}] at (skip_sum-east) 
    {Box={
        name=soft1,
        caption=SIGMOID,
        zlabel=192,
...
\draw [connection]  (end_b9-east)    -- node {\midarrow} (skip_sum-west);

\draw [connection]  (skip_sum-east)    -- node {\midarrow} (soft1-west);
